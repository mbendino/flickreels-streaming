...
import { useChapters, useStream } from '../hooks/useDramas';
...
const Watch = () => {
  ...
  const { chapters, detail, loading } = useChapters(id);
  const { streamUrl, loadStream } = useStream();

  useEffect(() => {
    if (id && chapters.length > 0) {
      loadStream(id, currentEpisode + 1);
    }
  }, [id, chapters, currentEpisode, loadStream]);
  ...
  const handleNext = () => {
    if (currentEpisode < chapters.length - 1) {
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="p-4 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleNext} disabled={currentEpisode === chapters.length - 1} className="flex-1 btn-primary py-3 rounded-lg font-medium disabled:opacity-50 flex items-center justify-center gap-2">
